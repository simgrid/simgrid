...
TEST_CASE("Activity lifecycle: direct communication (sendto) activities")
{
  ...
  BEGIN_SECTION("sendto")
  {
    ...
    simgrid::s4u::ActorPtr sendto5 =
        simgrid::s4u::Engine::get_instance()->add_actor("sendto5", all_hosts[1], [&global]() {
          assert_exit(true, 5.);
  ...
  BEGIN_SECTION("sendto actor killed at start")
  {
    ...
    simgrid::s4u::ActorPtr sendto5 =
        simgrid::s4u::Engine::get_instance()->add_actor("sendto5_killed", all_hosts[1], []() {
          assert_exit(false, 0);
          simgrid::s4u::Comm::sendto(all_hosts[1], all_hosts[2], 5000);
  ...
  BEGIN_SECTION("sendto host restarted at start")
  {
    ...
    simgrid::s4u::ActorPtr sendto5 =
        simgrid::s4u::Engine::get_instance()->add_actor("sendto5_restarted", all_hosts[1], []() {
          assert_exit(false, 0);
          simgrid::s4u::Comm::sendto(all_hosts[1], all_hosts[2], 5000);
          FAIL("I should be dead now");
        });
  ...
  BEGIN_SECTION("sendto host restarted at end")
  {
    ...
    simgrid::s4u::Engine::get_instance()->add_actor("sendto5_restarted", all_hosts[1], [&execution_done]() {
      assert_exit(true, 5);
      simgrid::s4u::Comm::sendto(all_hosts[1], all_hosts[2], 5000);
      execution_done = true;
    });
