...
static network_link_DASSF_t **routing_table = NULL;
...
{
  routing_table = xbt_new0(network_link_DASSF_t *, card_number * card_number);
...
{
  network_link_DASSF_t nw_link = xbt_new0(s_network_link_DASSF_t, 1);
...
{
  return ((network_card_DASSF_t) resource_id)->name;
}
...
static double share_resources(double now)
{
  ...
  surf_action_network_DASSF_t action = NULL;
...
static void update_resource_state(void *id,
				  ...
				  double value)
{
  network_link_DASSF_t nw_link = id;
...
static void action_suspend(surf_action_t action)
{
  ...
  lmm_update_variable_weight(maxmin_system,
			     ((surf_action_network_DASSF_t) action)->variable, 0.0);
}

static void action_resume(surf_action_t action)
{
  lmm_update_variable_weight(maxmin_system,
			     ((surf_action_network_DASSF_t) action)->variable, 1.0);
  ((surf_action_network_DASSF_t) action)->suspended = 0;
}

static int action_is_suspended(surf_action_t action)
{
  return ((surf_action_network_DASSF_t) action)->suspended;
}
...
/***************************************************************************/
/* The nice Hack by Henri to use DASSF from within SURF !                  */
/***************************************************************************/
/* I put this prototype here for now but it will have to go in 
   src/include/surf.h when it will be functionnal. */
void surf_network_resource_init_DASSF(const char *filename);
