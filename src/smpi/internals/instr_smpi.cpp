...
void TRACE_smpi_init(int rank)
{
  ...
  for (auto& it : counters) {
     ...
     */
    if (s_type::PJ_type_get_or_null(it.first.c_str(), container->type) == nullptr) {
...
void TRACE_smpi_collective_in(int rank, const char *operation, instr_extra_data extra)
{
  ...
  type_t type = s_type::PJ_type_get ("MPI_STATE", container->type);
...
void TRACE_smpi_send(int rank, int src, int dst, int tag, int size)
{
  ...
  container_t container = PJ_container_get (str);
  type_t type = s_type::PJ_type_get ("MPI_LINK", PJ_type_get_root());
