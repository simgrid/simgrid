...
#else
static pthread_t __current_thread_id;
#endif
...
{
	#ifdef WIN32
	return (GetCurrentThreadId() == __current_thread_id) ? 1 : 0;
	#else
	return (pthread_self() == __current_thread_id) ? 1 : 0;
	#endif
}
...
__context_wrapper(void *c)
{
	xbt_context_t context = current_context;
	  ...
	  DEBUG4("**[ctx:%p;self:%p]** Releasing the creator (creation_cond %p,%p) ****",context, (void *) xbt_os_thread_self(), creation_cond,creation_mutex);
	  xbt_os_cond_signal(creation_cond);
	  xbt_os_mutex_unlock(creation_mutex);

	  DEBUG4("**[ctx:%p;self:%p]** Going to Jail on lock %p and cond %p ****",context, (void *) xbt_os_thread_self(), context->mutex,context->cond);
	  xbt_os_cond_wait(context->cond, context->mutex);

	  DEBUG3("**[ctx:%p;self:%p]** Unlocking individual %p ****",context, (void *) xbt_os_thread_self(), context->mutex);
...
xbt_context_start(xbt_context_t context)
{
	...
	/* Launch the thread */
	  ...
	  DEBUG4("**[ctx:%p;self:%p]** Going to jail on creation_cond/mutex (%p,%p) ****",context, xbt_os_thread_self(), creation_cond, creation_mutex);
...
{
	xbt_context_t context = NULL;
	DEBUG1("Emptying trashbin (%d contexts to free)",xbt_swag_size(context_to_destroy));
...
__xbt_context_yield(xbt_context_t context)
{
	...
	{
		DEBUG1("--------- current_context (%p) is yielding back to maestro ---------",context);
	} 
	else 
	{
