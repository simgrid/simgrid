...
template<typename T>
std::vector<T>* wrap_returned_mpi_alltoall(std::vector<T> data, int sendcount, MPI_Datatype sendtype, int recvcount, MPI_Datatype recvtype, MPI_Comm comm){
...
template<typename T>
void wrap_mpi_alltoall(py::array_t<T> data, int sendcount, MPI_Datatype sendtype, py::array_t<T> &output, int recvcount, MPI_Datatype recvtype, MPI_Comm comm){
...
template<typename T>
void wrap_mpi_bcast(py::array_t<T> output, int count, MPI_Datatype type, int root, MPI_Comm comm){
     py::buffer_info out_buffer = output.request();
     auto *output_ptr = static_cast<T *>(out_buffer.ptr);
     MPI_Bcast(output_ptr, count, type, root, comm);
}
template<typename T>
void wrap_mpi_reduce_scatter(py::array_t<T> input, py::array_t<T> output, std::vector<int> count, MPI_Datatype type, MPI_Op op, MPI_Comm comm){
     py::buffer_info out_buffer = output.request();
     auto *output_ptr = static_cast<T *>(out_buffer.ptr);
     MPI_Reduce_scatter(input.data(), output_ptr, count.data(), type, op, comm);
}
template<typename T>
void wrap_mpi_reduce_scatter_block(py::array_t<T> input, py::array_t<T> output, int count, MPI_Datatype type, MPI_Op op, MPI_Comm comm){
     py::buffer_info out_buffer = output.request();
     auto *output_ptr = static_cast<T *>(out_buffer.ptr);
     MPI_Reduce_scatter_block(input.data(), output_ptr, count, type, op, comm);
}
template<typename T>
void wrap_mpi_reduce(py::array_t<T> input, py::array_t<T> output,int count, MPI_Datatype type, MPI_Op op, int root, MPI_Comm comm){
     py::buffer_info out_buffer = output.request();
     auto *output_ptr = static_cast<T *>(out_buffer.ptr);
     MPI_Reduce(input.data(), output_ptr, count, type, op, root, comm);
}
template<typename T>
void wrap_mpi_allreduce(py::array_t<T> input, py::array_t<T> output,int count, MPI_Datatype type, MPI_Op op, MPI_Comm comm){
     py::buffer_info out_buffer = output.request();
     auto *output_ptr = static_cast<T *>(out_buffer.ptr);
     MPI_Allreduce(input.data(), output_ptr, count, type, op, comm);
}
template<typename T>
void wrap_mpi_allgather(py::array_t<T> input, int send_count, MPI_Datatype send_type, py::array_t<T> output,int output_count,  MPI_Datatype output_type, MPI_Comm comm){
     py::buffer_info out_buffer = output.request();
     auto *output_ptr = static_cast<T *>(out_buffer.ptr);
...
PYBIND11_MODULE(simgrid, m)
{
  ...
  /* smpi namespace */
  auto smpi_m = m.def_submodule("smpi", "Bindings of the s4u::smpi namespace. See the C++ documentation for details.")
      ...
      .def("thread_create", &SMPI_thread_create,  py::call_guard<py::gil_scoped_release>(), "Initialize the SMPI interface with threads")
      .def("main", [](const std::string& name, std::vector<std::string> args) {
      ...
      .def(
          ...
          [](const std::string& name, std::vector<Host*> const hosts, py::object fun, py::args args) {
            ...
            const py::gil_scoped_release gil_release;
            return SMPI_app_instance_start(name.c_str(), [fun_p = fun.ptr(), args_p = args.ptr()]() {
      ...
      .def(
          ...
          [](const std::string& name, py::object fun, std::vector<Host*> const hosts) {
            fun.inc_ref();  // keep alive after return
            const py::gil_scoped_release gil_release;
            return SMPI_app_instance_start(name.c_str(), [fun_p = fun.ptr()]() {
              const py::gil_scoped_acquire py_context;
              try {
                const auto fun  = py::reinterpret_borrow<py::object>(fun_p);
                fun();
   ...
   py::class_<simgrid::smpi::Comm, std::unique_ptr<simgrid::smpi::Comm, py::nodelete>> mpi_comm(
     ...
     mpi_comm.def_property_readonly_static("WORLD", [](py::object /* self */) { return (simgrid::smpi::Comm*)MPI_COMM_WORLD; });
     mpi_comm.def_property_readonly_static("SELF", [](py::object /* self */) { return (simgrid::smpi::Comm*)MPI_COMM_SELF; });
     ...
     mpi_m.def("Alltoall", wrap_returned_mpi_alltoall<int>,  py::call_guard<py::gil_scoped_release>(), "Alltoall operation");
     mpi_m.def("Alltoall", wrap_returned_mpi_alltoall<double>,  py::call_guard<py::gil_scoped_release>(), "Alltoall operation");
     mpi_m.def("Alltoall", wrap_returned_mpi_alltoall<float>,  py::call_guard<py::gil_scoped_release>(), "Alltoall operation");
     ...
     mpi_m.def("Reduce_scatter", wrap_mpi_reduce_scatter<int>,  py::call_guard<py::gil_scoped_release>(), "Reduce scatter operation");
     mpi_m.def("Reduce_scatter", wrap_mpi_reduce_scatter<double>,  py::call_guard<py::gil_scoped_release>(), "Reduce scatter operation");
     mpi_m.def("Reduce_scatter", wrap_mpi_reduce_scatter<float>,  py::call_guard<py::gil_scoped_release>(), "Reduce scatter operation");
     mpi_m.def("Reduce_scatter_block", wrap_mpi_reduce_scatter_block<int>,  py::call_guard<py::gil_scoped_release>(), "Reduce scatter block operation");
     mpi_m.def("Reduce_scatter_block", wrap_mpi_reduce_scatter_block<double>,  py::call_guard<py::gil_scoped_release>(), "Reduce scatter block operation");
     mpi_m.def("Reduce_scatter_block", wrap_mpi_reduce_scatter_block<float>,  py::call_guard<py::gil_scoped_release>(), "Reduce scatter block operation");
   ...
   mpi_op
     .def_readonly_static("MAX", MPI_MAX)
  ...
  py::class_<MPI_Status>(mpi_m, "MPI_Status")
   ...
   py::class_<Engine>(m, "Engine", "Simulation Engine")
       ...
       .def_property_readonly("all_hosts", &Engine::get_all_hosts,
                              "Returns the list of all hosts found in the platform")
       .def_property_readonly("all_links", &Engine::get_all_links,
                              "Returns the list of all links found in the platform")
   ...
   netzone.def_static("create_full_zone", &simgrid::s4u::create_full_zone, "Creates a zone of type FullZone")
       ...
       .def_property("parent", &simgrid::s4u::NetZone::get_parent, &simgrid::s4u::NetZone::set_parent,
                     "NetZone parent (r/w property).")
   ...
   host.def_static("by_name", &Host::by_name, py::arg("name"), "Retrieves a host from its name, or die")
       ...
       .def(
           ...
           "The second function parameter is the periodicity: the time to wait after the last event to start again "
           "over "
   ...
   link.def("set_latency", py::overload_cast<const std::string&>(&Link::set_latency),
       ...
       .def(
           ...
           "For example, the following content describes a link which latency changes to 1ms (0.001s) at t=1, and to "
           "2s "
           "at t=2:\n\n"
           ".. code-block:: python\n\n"
           "   \"\"\"\n"
   ...
   py::class_<Comm, CommPtr, Activity>(m, "Comm", "Communication. See the C++ documentation for details.")
       ...
       .def("start", &Comm::start, py::call_guard<py::gil_scoped_release>(),
       ...
       .def_static(
           "sendto_async", &Comm::sendto_async, py::call_guard<py::gil_scoped_release>(), py::arg("from"),
   ...
   py::class_<simgrid::s4u::Exec, simgrid::s4u::ExecPtr, Activity>(m, "Exec",
       ...
       .def("suspend", &simgrid::s4u::Exec::suspend, py::call_guard<py::gil_scoped_release>(),
   ...
   py::class_<Semaphore, SemaphorePtr>(m, "Semaphore",
       ...
       .def("acquire_timeout", &Semaphore::acquire_timeout, py::call_guard<py::gil_scoped_release>(),
            py::arg("timeout"),
       ...
       .def_property_readonly(
           "would_block", &Semaphore::would_block, py::call_guard<py::gil_scoped_release>(),
