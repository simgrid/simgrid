 ...
 * typically used).
 *
 * Support, however, needs to be added to clock vectors since SimGrid permits the *creation* of new actors during
 * execution. Since we don't know this size before-hand, we have to allow clock vectors to behave as if they were
 * "infinitely" large. To do so, all newly mapped elements, if not assigned a value, are defaulted to `0`. This
 * corresponds to the value this actor would have had regardless had its creation been known to have evnetually
 * occurred: no actions taken by that actor had occurred prior, so there's no way the clock vector would have been
 * updated. In other words, when comparing clock vectors of different sizes, it's equivalent to imagine both of the same
 * size with elements absent in one or the other implicitly mapped to zero.
 */
...
public:
  ...
  ClockVector(Clock initial_value) : contents_(std::vector<Clock>(max_size, initial_value)) {}
   ...
   *
   * A `ClockVector` implicitly maps the id of an actor it does not contain to a default value of `0`. Thus, a
   * `ClockVector` is "lazy" in the sense that new actors are "automatically" mapped without needing to be explicitly
   * added the clock vector when the actor is created. This means that comparison between clockO vectors is possible
   * even as actors become enabled and disabled
   *
   * @return uint32_t the number of elements in the clock vector
   */
  size_t size() const { return this->contents_.size(); }

  Clock& operator[](Aid aid)
  {
    // NOTE: The `operator[]` overload of unordered_map will create a new key-value pair if `tid` does not exist and
    // will use a _default_ value for the value (0 in this case) which is precisely what we want here
