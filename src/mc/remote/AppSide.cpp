...
#if HAVE_MALLOPT
    // Configure malloc to try to reduce the amount of page creations
    mallopt(M_MMAP_THRESHOLD, 64 * 1024 * 1024); // Don't use mmap for block smaller than 64M (default is 128k)
...
void AppSide::handle_fork(const s_mc_message_fork_t* msg)
{
  ...
  if (first_time && std::strcmp(simgrid::s4u::Engine::get_instance()->get_context_factory_name(), "thread") == 0) {
    ...
    xbt_die("The SimGrid model-checker cannot handle the threaded context factory because it relies on forking the "
            ...
            "simgrid-mc. The second option results in slightly slower explorations, but it is the only option when "
            "verifying programs that mandate threads (such as Python and Java).");
