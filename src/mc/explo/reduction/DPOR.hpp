...
class DPOR : public Reduction {
  ...
  std::optional<EventHandle> max_dependent_dpor(const odpor::Execution& S, EventHandle limit, const State* s, Aid p);

  std::unordered_set<Aid> compute_ancestors(const odpor::Execution& S, EventHandle limit, stack_t* state_stack, Aid p,
                                            EventHandle i);
...
public:
  ...
  class RaceUpdate : public Reduction::RaceUpdate {
    std::vector<std::pair<StatePtr, std::unordered_set<Aid>>> state_and_ancestors_ =
        std::vector<std::pair<StatePtr, std::unordered_set<Aid>>>();

  public:
    RaceUpdate() = default;

    void add_element(StatePtr state, std::unordered_set<Aid> ancestors)
    {
      state_and_ancestors_.emplace_back(state, ancestors);
    }
    std::vector<std::pair<StatePtr, std::unordered_set<Aid>>>& get_value() { return state_and_ancestors_; }
  };
