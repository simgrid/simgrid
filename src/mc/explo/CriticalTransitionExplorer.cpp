...
void CriticalTransitionExplorer::log_stack()
{
  ...
  for (const auto& [state, out_transition] : initial_bugged_stack) {
    ...
    XBT_INFO("  (%s) Actor %d in %s ==> simcall: %s", status.c_str(), out_transition->aid_.c_val(),
...
void CriticalTransitionExplorer::run()
{
  ...
  while (stack_->size() > 1 and not stack_->back()->has_correct_execution()) {
    ...
    try {
      XBT_DEBUG("Running the DFS exploration from after transition Actor %d : %s", current_candidate->aid_.c_val(),
      ...
      XBT_INFO("Found the critical transition: Actor %d ==> simcall: %s", current_candidate->aid_.c_val(),
  ...
  XBT_INFO("Found the critical transition: Actor %d ==> simcall: %s", critical_transition->aid_.c_val(),
