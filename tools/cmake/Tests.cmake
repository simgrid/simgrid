...
IF(enable_memcheck)
  ...
  if (NOT VALGRIND_EXE MATCHES "NOTFOUND")
    ...
    if(enable_memcheck_xml)
      SET(VALGRIND_WRAPPER ${VALGRIND_WRAPPER}\ --xml=yes\ --xml-file=valgrind_test_%p.xml\ --child-silent-after-fork=yes\ )
    endif()
...
MACRO(ADD_TESH NAME)
  ...
  if(VALGRIND_WRAPPER)
    set(wrapper ${VALGRIND_WRAPPER}\ --xml-user-comment=Test_${NAME}\ )
    ADD_TEST(NAME ${NAME} COMMAND ${TESH_COMMAND} --wrapper "${wrapper}" ${TESH_OPTION} ${ARGT})
    unset(wrapper)
  else()
    ADD_TEST(NAME ${NAME} COMMAND ${TESH_COMMAND} ${TESH_OPTION} ${ARGT})
  endif()
ENDMACRO()
...
if(SIMGRID_HAVE_SMEMORY)
  ...
  set(EXTRA_DIST ${EXTRA_DIST} ${SMEMORY_UNIT_TESTS})
endif()
...
if(SIMGRID_HAVE_SMEMORY)
  set(UNIT_TESTS ${UNIT_TESTS} src/mc/smemory/MemoryAccessTracker_test.cpp)
else()
  set(EXTRA_DIST ${EXTRA_DIST} src/mc/smemory/MemoryAccessTracker_test.cpp)
endif()
...
if(SIMGRID_HAVE_SMEMORY)
  target_link_libraries(unit-tests xxhash)
endif()
