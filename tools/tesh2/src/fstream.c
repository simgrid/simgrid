...
int  fstream_parse(fstream_t fstream, xbt_os_mutex_t mutex) 
{
  ...
  int to_be_continued;
      ...
      while (!(unit->root->interrupted)
        ...
        ERROR1("[%s] Error : no command found in the last chunk of lines",
                file_pos);
...
fstream_lex_line(fstream_t fstream, context_t context,
                 xbt_os_mutex_t mutex, const char *filepos, char *line) 
{
      ...
      ERROR2("[%s] Malformated suite `(%s)': include missing", filepos,
              (*current_suite)->description);
    
    else
      ERROR2("[%s] Malformated suite `(%s)': blank line missing", filepos,
              (*current_suite)->description);
       ...
       */ 
        ...
        if ((p = strstr(line2 + 2, buff)))
          ...
          ERROR2("[%s] Bad substitution : `%s'.", filepos,
                    strstr(buff, "${"));
        ...
        str_replace_all(&line2, exp, variable->val ? variable->val : val,
                          NULL);
         ...
         */ 
    {
          ...
          ERROR2("[%s] Syntax error : `%s'.", filepos,
                  strstr(line2, "${"));
        ...
        ERROR1("[%s] Missing space after `$' `(usage : $ <command>)'",
                filepos);
        ...
        if (!strncmp("cd ", line2 + 2, 3))
          ...
          while (*(dir++) == ' ');
            ...
            ERROR3("[%s] Chdir to %s failed: %s", filepos, dir,
      ...
      fstream_process_token(fstream, context, mutex, filepos, line2[0],
                             line2 + 2);
      ...
      WARN2("[%s] Description already specified `%s'", filepos,
             line2 + 2);
                      ...
                      char token, char *line) 
{
      ...
      VERB1("[%s] More than one command in this chunk of lines",
              filepos);
          ...
          ERROR1("[%s] Undefinite command for `$' `(usage: $ <command>)'",
                  filepos);
...
#ifdef _XBT_WIN32
    {
      ...
      if (!is_w32_cmd(command_line, fstream->unit->runner->path)
            && getpath(command_line, &path) < 0)
         {
        command_t command;
        ERROR3("[%s] `%s' : NOK (%s)", filepos, command_line,
          ...
          sprintf(context->t_command_line, "%s\\%s", path,
        ...
        ERROR2("[%s] Signal `%s' not supported by this platform", filepos,
                context->signal);
          ...
          if (!sig_exists(context->signal))
        ...
        ERROR2("[%s] Signal `%s' not supported by Tesh", filepos,
                context->signal);
...
#endif  /*  */
          VERB2("[%s] (next command must raise signal %s)", filepos,
                  context->signal);
      ...
      VERB2("[%s] (next command must return code %d)", filepos,
             context->exit_code);
        ...
        fstream_handle_include(fstream, context, mutex, file_name,
                                 p2[0] != '\0' ? p2 : NULL);
            ...
            ERROR2("[%s] A system variable named `(%s)' already exists",
                    filepos, name);
            ...
            WARN2("[%s] Variable `(%s)' already unseted", filepos,
                   variable->name);
        ...
        ERROR2("[%s] `(%s)' is system variable : you can unset it",
                filepos, name);
            ...
            if (exists)
              ...
              WARN3("[%s] Variable `(%s)' already contains value `<%s>'",
                       ...
                       const char *description) 
{
      ...
      if (!_fstream)
        ...
        ERROR2("[%s] Include file `(%s)' not found", context->line,
                file_name);
                       ...
                       xbt_os_mutex_t mutex) 
{
        ...
        ERROR3("[%s] Cannot instantiate the command `%s' (%d)",
      ...
      ERROR3("[%s] Cannot run the command `%s' (%d)", context->pos,
    ...
    ERROR3("[%s] Cannot reset the context of the command `%s' (%d)",
            context->pos, strerror(errno), errno);
