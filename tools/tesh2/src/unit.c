...
unit_start(void* p) 
{
	...
	if(!root->interrupted)
	{
		 ...
		 */
		if(!root->released && (root->started_cmd_nb == (root->failed_cmd_nb + root->interrupted_cmd_nb + root->successeded_cmd_nb)))
...
unit_new(runner_t runner, unit_t root, unit_t owner, fstream_t fstream)
{
	...
	unit->commands = xbt_dynar_new(sizeof(command_t), (void_f_pvoid_t)command_free);
	
	/* instantiate the vector used to store all the included units */
	unit->includes = xbt_dynar_new(sizeof(unit_t), (void_f_pvoid_t)unit_free);
	
	/* instantiate the vector used to store all the included suites */
	unit->suites = xbt_dynar_new(sizeof(unit_t), (void_f_pvoid_t)unit_free);
