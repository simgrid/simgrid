#include <xerrno.h>
  ...
  {"EXDEV", EXDEV, 0, "cross-device link"}, 
      ...
      {"ESIGNOTRECEIPT", ESIGNOTRECEIVED, 1,
       "expected signal not receipt"}, {"EPROCCMDLINE", EPROCCMDLINE, 1,
                                          "command line processing failed"},
      {"ENOARG", ENOARG, 1, "none optional argument not specified"},
      {"ENOTPOSITIVENUM", ENOTPOSITIVENUM, 1,
         "argument option not strictly positive"}, {"ESYNTAX", ESYNTAX,
                                                      1, "syntax error"},
      {"ELEADTIME", ELEADTIME, 1, "timed out"}, {"EREADMENOTFOUND",
                                                     EREADMENOTFOUND, 1,
                                                     "unable to locate the README.txt file"},
      {"EINCLUDENOTFOUND", EINCLUDENOTFOUND, 1,
         "include file not found"}, {"ESUFFIXTOOLONG", ESUFFIXTOOLONG, 1,
                                       "suffix too long"},
...
static char * w32error_to_string(DWORD errcode) 
{
      ...
      FormatMessage(FORMAT_MESSAGE_FROM_HMODULE, hModule, errcode,
