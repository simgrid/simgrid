...
void  command_exec(command_t command, const char *command_line) 
{
  ...
  HANDLE child_stdin_handle[2] = {
  NULL};                        /* child_stdin_handle[1]        <-> stdout of the child process */
  HANDLE child_stdout_handle[2] = {
  NULL};                        /* child_stdout_handle[0]       <-> stdin of the child process  */
    ...
    ERROR3("[%s] `%s' : NOK (%s)", command->context->pos,
            command->context->command_line,
      ...
      if (!CreateProcess
          (NULL, tow32cmd(command_line), NULL, NULL, TRUE,
              ...
              error_to_string((int) GetLastError(), 0));
                       ...
                       command->context->pos);
      command_handle_failure(command,
...
void  command_exec(command_t command, const char *command_line) 
{
  ...
  int rv =
    ...
    ERROR2("[%s] Cannot fork the command `%s'", command->context->pos,
             command->context->command_line);
...
void  command_wait(command_t command) 
{
      ...
      DWORD rv;
    ...
    ERROR2("[%s] Cannot wait for the child`%s'", command->context->pos,
            command->context->command_line);
        ...
        ERROR2("[%s] Cannot get the exit code of the process `%s'",
...
void  command_wait(command_t command) 
{
    ...
    ERROR2("[%s] Cannot execute the command `%s'", command->context->pos,
            command->context->command_line);
...
void  command_check(command_t command) 
{
    ...
    ERROR3("[%s] `%s' : NOK (unexpected signal `%s' caught)",
            command->context->pos, command->context->command_line,
            command->signal);
    ...
    ERROR4("[%s] `%s' : NOK (got signal `%s' instead of `%s')",
             command->context->pos, command->context->command_line,
    ...
    ERROR3("[%s] `%s' : NOK (expected `%s' not received)",
             command->context->pos, command->context->command_line,
             command->context->signal);
    if (success)
       {
      success = 0;
      unit_set_error(command->unit, ESIGNOTRECEIVED, 1,
                      command->context->pos);
      ...
      ERROR4("[%s] %s : NOK (returned code `%d' instead `%d')",
              command->context->pos, command->context->command_line,
    ...
    if (command->output->used != command->context->output->used
         || strcmp(command->output->data, command->context->output->data))
       {
      char *diff;
      ERROR2("[%s] `%s' : NOK (outputs mismatch):",
                command->context->pos, command->context->command_line);
    ...
    INFO1("(ignoring the output of <%s> as requested)",
...
void  command_kill(command_t command) 
{
    ...
    INFO2("[%s] Kill the process `%s'", command->context->pos,
           command->context->command_line);
...
void  command_summarize(command_t command) 
{
  ...
  if (csr_command_not_found != command->reason
